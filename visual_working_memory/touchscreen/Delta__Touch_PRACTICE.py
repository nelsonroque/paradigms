...
class Instructions:
    def __init__(self,headline_text,headline_height,current_instruct,instructs_list,instruct_height,font_color):
        ...
        self.continue_text = visual.TextStim(win,text="TOUCH ANYWHERE ON SCREEN TO CONTINUE", height=self.continue_height, color=self.font_color, pos=[0,-500])
    ...
    def drawAll(self):
        for current_instruction in self.INSTRUCTS_LIST:
            ...
            touchResponse = False
            while(not touchResponse):
                mouse_coords = mymouse.getPos()
    ...
    while(TRIAL <= len(block_structure)):
        ...
        while(ResponseGiven == False and elapsed < timeout_duration):
            ...
            if(elapsed > timeout_duration):
                ...
                if mymouse.mouseMoved():
                    for i, square in enumerate(response_list): # get each shape and its index number
                        if square.contains(mymouse):
                            isObjTarget = response_ids[i]
                            isTimeoutTrial = 0
                            end_time = clock()
                            ...
                            ResponseGiven = True
            mouse_pos = mymouse.getPos()
            if(mouse_pos[0] >= (0-(screen_width/2))) and (mouse_pos[1] >= (0+(screen_height/2))):
                core.quit()
...
filename = "obj/practice/12/objdata" + "_" + str(subject_number) + ".txt"
data_file = "data/practice/12/data" + "_" + str(subject_number) + ".txt"
...
if DEBUG_CODE == 0 or DEBUG_CODE == 2:
    # instructions
    instructs_list = ConfigData("prac_instruct_list").getValue()
    ...
    block_data = genBlock(NUM_PRACTICE_TRIALS_PER_CONDITION,12,trials_types,1)

    # run the practice block
    runBlock(0, block_data, timeout_duration, filename, data_file)
