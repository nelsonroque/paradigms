...
def runBlock(block_num, block_structure, timeout_duration, filename, data_file, response_keys):
    BLOCK = block_num
    TRIAL = 1
    ...
    while(TRIAL <= len(block_structure)):
        ...
        win.flip()
        start_time = clock()
        k = event.waitKeys(maxWait=timeout_duration,keyList=response_keys)
        end_time = clock()

        # calculate RT
        RT = end_time - start_time
        
        if RT > timeout_duration:
            Click_X = "NA"
            Click_Y = "NA"
            isTimeoutTrial = 1
            isObjTarget = "NA"
            ...
            Click_Y = k[0]
            isTimeoutTrial = 0
            if k[0] == response_keys[0]:
                isObjTarget = response_ids[0]
            elif k[0] == response_keys[1]:
...
print("PARTICIPANT:",subject_number)
...
response_keys = ConfigData("response_keys").getValue()
...
if DEBUG_CODE == 0 or DEBUG_CODE == 2:
    ...
    runBlock(0, block_data, timeout_duration, filename, data_file, response_keys)