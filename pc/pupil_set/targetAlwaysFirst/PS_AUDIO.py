...
def playAudioCue(TARG_COLOR):
    ...
    # generate filename for audio to play
    audioToPlay = 'sounds/' + TARG_COLOR + '.wav'
    audi = sound.Sound(audioToPlay)
...
def single_trial(VARIANT, PARTICIPANT, TODAY, TRACKER_CONNECTED, TRIAL_STRING, trial_count, total_stream_length, stream_speed):
    ...
    CONTINUE_TIME = playAudioCue(TARG_COLOR)
...
if(CAL_SUCCESS):
    ...
    # write file header
    df.write("VARIANT,PARTICIPANT,DATE,TRIAL_START_SYS_TIME,TRIAL,STREAM_SPEED_S,TARG_INDEX,TARG_COLOR,DIST_PRE_TARG_IDEN,TARG_IDEN,DIST_POST_TARG_IDEN,RESPONSE_IDEN,ACC,DOES_RESP_EQUAL_DIST_PRE_TARG,DOES_RESP_EQUAL_TARG,DOES_RESP_EQUAL_DIST_POST_TARG,RT,SPACE_TO_CONTINUE_TIME,LETTER_STR,COLOR_STR\n")
    df.close()
